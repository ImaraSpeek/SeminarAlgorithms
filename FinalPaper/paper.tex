...
For a planner to be succesfull in finite-horizon MDPs they need to be scalable and sensitive to an expected reward maximization, the evaluation criteria of these domains. This caused for  LRTDP planners to emerge.
...
A \emph{Labeled Reverse Real-Time Dynamic Programmer} (LR\textsuperscript{2}TDP) uses \emph{reverse iterative deepening} and LRTDP-FH to incrementally solve a sequence of MDPs $M(1)$, $M(2)$, \ldots , $M(H)$ in order by relying on the fact that a successful policy can be produced by solving an MDP-FH $M(H)$ for a smaller horizon $h < H$. A LR\textsuperscript{2}TDP\@ iterates on the distance from the horizon and allows for reusing precomputed state-action plans from converged states. However when dealing with large branching factors Bellman computations tend to become prohibitively expensive. 
...
\textsc{GLUTTON} introduces an off-line algorithm based on LR\textsuperscript{2}TDP. The \emph{Max-Reward} heuristic used to evaluate \todo{the what?} relies on knowing the maximum reward $R_{max}$ any action can yield in a state or an upper bound on it. In addition to LR\textsuperscript{2}TDP, it introduces the following mechanisms to optimize for large branching factors:
...
Because real world domains contain Natural Dynamics that introduce large branching factors, offline planning is often a waste of time and memory space since there is a vast amount of states the planner may never reach in a lifetime. Such problems are far better solved using online planners by finding an optimal policy $\tau$* or an approximation of this polict for the \emph{current} state, executing it and repeating. This is done by choosing a certain lookahead value $L$ and solving the MDP \emph{M\textsubscript{s}(L)} to solve \emph{M\textsubscript{0}(H)} where L $<$ H and then select \emph{$\pi$*\textsubscript{L}(s, L)} using the evaluation criteria. This lookahead value is determined using the per-process time constraint.\todo{elaborate on the per-process time constraint}
