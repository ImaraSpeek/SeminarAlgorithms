...
\emph{Uncertainty applies to predictions of future events, to physical measurements that are already made, or to the unknown. Uncertainty arises in partially observable and/or stochastic environments, as well as due to ignorance and/or indolence}
...
Meet my plant
    ...
    \includegraphics[scale=0.08]{images/plant.jpg}
\end{frame}
...
''We have presented an approach to domain independent planning that, \emph{at the time being}, outperforms all existing technology on the majority of the currently available benchmark domains.'' 
...
Insert image on search state, enforced hill climbing, call relaxed graphplan, inform search with goal distance estimate and maybe promising succesors, solution plan or fail, possible search from scratch using hast tabel with visited states 
\end{frame}
...
Perks
	\begin{itemize}
		...
		\item Most domains in the IPCC are simple in their structure and can be easily solves using gree algorithms like FF
	\end{itemize}
