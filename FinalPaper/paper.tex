...
Since the first incorporation of the International Probabilistic Planning
Competition (IPPC), it has undergone radical changes. Dominating planners
of previous incorporations had a strong focus on goal-oriented MDPs,
determinizing and using powerful classical planners (e.g. FF
\cite{Hoffmann01theff}) and replanning. New benchmarks have demonstrated
weaknesses in existing solution techniques.

Little and Thi\'ebaux have developed a baseline test in
\cite{little2007probvsreplan} to evaluate the IPC-5 problems. This
test determines if a planning problem is `probabilistically interesting'.
Problems that fail this test are classical planning problems in disguise.
Knowing this it is easy to design a problem where a replanner performs badly
or incredibly well.
...
algorithms to solve those probabilistic complex problems, one must not forget
if the problem makes sense in the real world.

The main contribution of this paper is an evaluation of the domains used in
IPPC-11 and see if they can be adjusted so they fail the baseline test for
probabilistic interestingness, which might make them more suitable for
replanning planners.
