...
This paper presents a new domain involving Unmanned Aerial Vehicles (UAVs) that have to cooperate to deliver data packages from one UAV to the base station. To match this problem with an appropriate planner, this paper presents an overview of other interesting domains and probabilistic planners that were involved in the redefinition of an interesting planning problem. By evaluating the domains with the help of the probabilistic interestingness baseline test introduced by Little et al. \cite{little2007probvsreplan} a recommendation will be made for a suitable planner to match the UAV problem.
%This paper presents an overview of interesting domains and concurrent planners that were involved in the redefinition of an interesting planning problem. An interesting currently relevant domain is proposed and evaluated amongst previously defined domains. By using the probabilistic interestingness baseline test as introduced by Little et al. \cite{little2007probvsreplan} a recommendation will be made for the optimal planner to match this problem.
...
%% TODO maybe UAVs can be on or of too, so it's forced to use a further UAV.
%% TODO what's also interesting is that Amazon.com uses drones for package deliveries
...
other domains used in the IPPC-11 competition and compare that with the UAV
domain. Since Little and Thi\'ebauw's baseline test for \emph{probabilistic
interestingness} of domains, domains of the IPPC competition have shifted to
those probabilistically interesting domains, which are difficult for
replanners. Often domains that don't pass this test are deemed to be classical
planners in disguise, favoring the use of replanners.
...
The IPPC-06 has an elevators domain, however we describe the latest IPPC-11
elevators domain. In this domain there are a number of elevators delivering
passengers to either the top or bottom floor. Potential passengers arrive at
any floor at a certain rate (depending on which floor). The elevators can only
move in the direction it is indicating, so passengers know whether to board or
not. The goal of the elevators is to transport as many as possible passengers
...
The IPPC-11 domains, Navigation, Traffic, Elevators and SysAdmin are finite-
horizon domains. Navigation, however, is actually a goal oriented domain, if
the goal is reached in the horizon. The method this is defined is that the goal
state is defined as an absorbing state, and there is no reward or cost
associated to staying in the goal state, until the horizon is reached.

\textsc{Gourmand} vastly outperformed IPPC-11 UCT-based probabilistic track
winner PROST by far on goal-oriented domains Navigation and Crossing Traffic
\cite{kolobov2012gourmand}. PROST however mildly outperforms \textsc{Gourmand}
on the Game of Life, SysAdmin and the Traffic domain as UCT does not need to
perform expensive Bellman Backups on these domains that include large branching
factors.

The reason \textsc{Gourmand} performs quite well on the navigation domain is
because it uses a dynamic lookahead.
